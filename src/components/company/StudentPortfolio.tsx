...
const StudentPortfolio: React.FC = () => {
  ...
  const getGPAColor = (gpa: number) => {
    if (gpa >= 3.5) return 'text-blue-700 bg-blue-50 border-blue-200';
    if (gpa >= 3.0) return 'text-blue-600 bg-blue-100 border-blue-200';
    if (gpa >= 2.5) return 'text-slate-700 bg-slate-100 border-slate-200';
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-xl font-bold text-slate-900">Student Portfolios</h3>
          <p className="text-slate-600 mt-1">Browse comprehensive verified portfolios of talented students</p>
        </div>
      ...
      {sortedStudents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <div key={student.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
            {/* Header */}
            <div className="p-6 border-b border-slate-200 bg-slate-50">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-blue-600 text-sm font-medium">{student.course}</p>
                  <div className="flex items-center space-x-2 mt-1">
                    <MapPin className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-500">{student.university}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              {/* Top Skills */}
              <div className="mb-4">
                <h5 className="text-sm font-semibold text-slate-900 mb-2">Top Skills</h5>
                <div className="flex flex-wrap gap-1">
                  {student.skills.slice(0, 6).map((skill, index) => (
                    <span key={index} className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              {student.certificates.filter(c => c.status === 'approved').length > 0 && (
                <div className="mb-4">
                  <h5 className="text-sm font-semibold text-slate-900 mb-2">Latest Achievement</h5>
              ...
              {student.activities.length > 0 && (
                <div className="mb-4">
                  <h5 className="text-sm font-semibold text-slate-900 mb-2">Featured Activity</h5>
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-blue-600 font-medium">{selectedStudent.course} - Year {selectedStudent.year}</p>
                    <p className="text-slate-500">{selectedStudent.email}</p>
                    <div className="flex items-center space-x-2 mt-2">
                      <MapPin className="w-4 h-4" />
                      <span className="text-sm text-slate-600">{selectedStudent.university}</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-slate-600">Verified Certificates</span>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-slate-600">Activity Hours</span>
                        <span className="inline-flex items-center px-2 py-1 rounded-full text-sm font-medium bg-blue-50 text-blue-700 border border-blue-200">
                  ...
                  {/* Skills */}
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    <h4 className="font-semibold text-slate-900 mb-3">Technical Skills</h4>
                ...
                <div className="lg:col-span-2 space-y-6">
                  {/* Academic Records */}
                  <div>
                    <h4 className="font-semibold text-slate-900 mb-4">Academic Performance</h4>
                      ...
                      {selectedStudent.academicRecords.map((record) => (
                        <div key={record.id} className="bg-white border border-slate-200 rounded-xl p-4">
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              <h5 className="font-medium text-slate-900">{record.semester} {record.year}</h5>
                  ...
                  {/* Certificates */}
                  <div>
                    <h4 className="font-semibold text-slate-900 mb-4">Verified Certificates</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        .map((cert) => (
                          <div key={cert.id} className="bg-white border border-slate-200 rounded-xl p-4">
                            <div className="flex items-start space-x-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <h5 className="font-medium text-slate-900 truncate">{cert.title}</h5>
                  ...
                  {/* Activities */}
                  <div>
                    <h4 className="font-semibold text-slate-900 mb-4">Activities & Experience</h4>
