  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 mb-6 shadow-sm">
        ...
        <div>
          <h3 className="text-xl font-semibold text-slate-900">Certificate Management</h3>
          <p className="text-sm text-slate-500 mt-1">Review and manage student certificates</p>
        </div>
        <div className="flex items-center space-x-2">
          ...
          <select
            ...
            className="px-3 py-2 border border-slate-200 rounded-lg bg-white text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
          >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {certificates.map((cert) => (
            <div key={cert.id} className="border border-slate-200 bg-white rounded-xl p-6 hover:border-blue-300 hover:shadow-sm transition-all duration-300">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-start space-x-4">
                  <div className="w-12 h-12 rounded-lg bg-blue-50 flex items-center justify-center text-xs font-bold text-blue-600 border border-blue-100">
                  ...
                  {cert.status === 'pending' && (
                    <>
                      <button
                        ...
                        className="flex items-center space-x-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-colors disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        disabled={processingId === cert.id}
                        className="flex items-center space-x-1 px-3 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 rounded-lg text-xs font-bold transition-colors disabled:opacity-50"
                      >
                        <X className="w-4 h-4" />
                        <span>Reject</span>
                      </button>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-xl shadow-2xl border border-slate-200">
            ...
            <div className="space-y-4">
              <div className="flex items-start space-x-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
                <div className="w-14 h-14 rounded-lg bg-blue-50 flex items-center justify-center text-xs font-bold text-blue-600 border border-blue-100">
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                ...
                {selectedCertificate.status === 'pending' && (
                  <div className="flex space-x-2">
                    ...
                    className="px-4 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 rounded-lg text-xs font-bold flex items-center space-x-1"
                    >
                      <X className="w-4 h-4" />
                      <span>Reject</span>
                    </button>
                    ...
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold flex items-center space-x-1"
                    >
                      <Check className="w-4 h-4" />
